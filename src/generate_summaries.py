"""Generate LLM summaries for the 24 eval CVEs under both bullet prompt versions.
...
baseline and persona bullet templates, and sends both to the Anthropic API at
a fixed model and temperature=0. Writes one record per generation to
v2_bullet/summaries/summaries_bullet.json (48 total). Resumable: a
(cve_id, prompt_version) pair already present in the output file is skipped,
not regenerated.

Same data-quality decisions as the v1 prose run (src/generate_summaries.py on
the prompt-prose branch), carried over unchanged:
- References carry no source/vendor label anywhere in the pipeline, so
  Reference lines are emitted as a bare URL with no parenthetical label.
...
PROMPT_PATHS = {
    "baseline": "v2_bullet/prompts/prompt-baseline_v2.txt",
    "persona": "v2_bullet/prompts/prompt-persona_v2.txt",
}
OUTPUT_PATH = "v2_bullet/summaries/summaries_bullet.json"
...
def append_methodology_log(models_used, prompt_hashes, written, skipped, failed,
    ...
    entry = []
    entry.append("")
    entry.append(f"## Stage 6c -- Baseline vs Persona Bullet-Format Summary Generation ({date.today().isoformat()})")
    entry.append("")
    entry.append("**Script:** `src/generate_summaries.py`")
    entry.append("**Output:** `v2_bullet/summaries/summaries_bullet.json`")
    entry.append("")
    entry.append(
        f"Ran both frozen bullet-format prompt templates (`prompt-baseline_v2.txt`, "
        f"`prompt-persona_v2.txt`) against all 24 eval CVEs, reusing the saved "
        f"neighbours from `data/retrieval_validation.json` (no re-querying ChromaDB). "
        f"Model and temperature were fixed across all calls: model "
        f"`{', '.join(sorted(models_used))}`, temperature {TEMPERATURE}."
    )
    entry.append("")
