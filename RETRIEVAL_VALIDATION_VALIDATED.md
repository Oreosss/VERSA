...
Nearest-neighbour distance across the 24 eval CVEs ranges from 0.0068 to 0.6224 (median 0.3418, 75th percentile 0.4032). 6 eval CVE(s) sit above the 75th percentile on nearest-neighbour distance and are flagged below as candidates for closer relevance review -- this is a statistical prompt for attention, not a pass/fail judgement. Whether these CVEs are actually poorly grounded depends on the same-class relevance assessment recorded in the per-CVE sections, not on distance alone.
...
This file presents evidence only. It does not compute or state an overall pass/fail verdict for retrieval quality or eval sample quality -- that judgement rests on the per-CVE same-class relevance assessments below plus the coverage grid. The per-CVE assessments were produced by an automated nearest-neighbour distance check that flagged outliers for attention, followed by a model-assisted same-class relevance review of each target against its retrieved neighbours.
...
- Embedding model: `all-MiniLM-L6-v2` (matches corpus-build model in `src/chroma_ingest.py`). General-purpose, not security-domain-tuned -- cosine distance is a rough signal, not ground truth, which is why the same-class relevance assessment below carries real weight.

## Grounding: per-CVE detail and same-class assessment

For each target, its description is compared against each neighbour's description to assess whether they describe the same class of vulnerability. The assessment below combines the automated nearest-neighbour distance signal with a same-class relevance review. Descriptions are printed in full (not truncated).
...
SAME-CLASS ASSESSMENT (automated distance check + model-assisted relevance review): borderline

NOTES: Neighbours span robot-controller DoS, memory corruption, code injection and local privilege escalation. All loosely in the unauthorised-action / code-execution space but not tightly the same improper-ACL-plus-command-execution class as the target. High distance (0.4324) reflects genuine class spread, not a clean match.
...
SAME-CLASS ASSESSMENT (automated distance check + model-assisted relevance review): yes

NOTES: Neighbours 1 and 2 are same-vendor GL.iNet auth-bypass / code-execution and command injection. Neighbours 3-5 drift to DoS, token logging and NETGEAR command injection, but the top matches carry the same product and network-device access-control class.
...
NOTES: Target is SQL injection. Only neighbour 1 is SQLi. Neighbours 2-5 all drift to cross-site scripting. Genuinely weak same-class support and thin SQLi coverage in the corpus. Retained deliberately as a realistic weak-retrieval case. One of the two weakest-grounded CVEs in the set.
...
NOTES: All five neighbours are McAfee products. Classes vary (cleartext transmission, privilege escalation, unquoted path, XXE) rather than exactly matching the target's deserialisation-to-reverse-shell, but the product and security-domain grounding is strong. Same-class on product family, looser on precise class.
...
NOTES: All five neighbours are Ivanti Connect Secure / Policy Secure. Top match is same-vendor code injection to remote code execution. Very strong product and injection/code-execution class grounding.
...
NOTES: All five neighbours are Confluence Data Center / Server. Classes span OGNL/RCE, DoS, improper authorization, but the product grounding is tight and several share the unauthenticated-code-execution core.
...
NOTES: All five neighbours are Palo Alto PAN-OS. Top match is same-vendor OS command-argument injection. Strong product grounding with an injection/command-execution core, some spread to DoS and info disclosure.
...
NOTES: Neighbours are mostly Apache HTTP Server issues (path traversal, source disclosure, RCE) plus one Dubbo SSRF. Same product and path-traversal-to-code-execution family. Borderline on a couple of neighbours but grounded overall.
...
NOTES: Distance is high (0.4206) but four of five neighbours are OS command injection on network devices via crafted parameters / shell metacharacters, the same class as the target. A clear case of a high-distance false alarm from the statistical threshold.
...
NOTES: Target is server-side template code execution via Twig. Only neighbour 5 (Craft CMS Twig template RCE) is truly same-class. Neighbours 1-4 scatter across Apache info disclosure, a Linux kernel memory bug, and GitLab template disclosure. Genuinely weak grounding. The second of the two weakest-grounded CVEs in the set.
...
NOTES: Target is a heap overflow. All five neighbours are heap / buffer overflow vulnerabilities, several leading to code execution. Clean same-class match despite mid-range distance.
...
NOTES: Target is path traversal. All five neighbours are path-traversal (CWE-22) vulnerabilities. Tight same-class match.
...
NOTES: Target is admin-interface code execution on Pulse Connect Secure. Top neighbour is a near-identical Pulse code-injection issue. Remaining neighbours are code-execution / buffer-overflow on network appliances. Strong grounding.
...
NOTES: Target is OS command injection on SonicWall SMA100. Neighbours are same-product path traversal plus SSL-VPN command injection, auth bypass and DoS on comparable appliances. Injection / access-control family, grounded.
...
NOTES: Target is sudoers privilege escalation via a crafted NSE script. Neighbours are command injection, permissions issues, auth bypass and code execution on network devices. Related privilege / command-execution family but not precisely the same sudoers-misconfiguration class. High distance (0.5283).
...
NOTES: Target is an Adobe Acrobat Reader use-after-free. All five neighbours are near-identical Adobe Acrobat Reader use-after-free entries at extremely low distance (0.0068). Textbook same-class match, the tightest in the set.
...
NOTES: Target is reflected XSS in a WordPress plugin. All five neighbours are WordPress-plugin XSS from unescaped parameters. Clean same-class match.
...
NOTES: Target is stored XSS via log files in a WordPress plugin. Four of five neighbours are WordPress XSS; one is CSRF. Strong same-class grounding.
...
NOTES: Target is a macOS privacy-preferences bypass logic issue. Neighbours are same-platform macOS 'logic issue / malicious application may bypass' entries. Very tight product and pattern match.
...
NOTES: Target is command injection in a Totolink device via cstecgi.cgi. All five neighbours are Totolink cstecgi.cgi command-injection / overflow issues. Same product and injection class.
...
NOTES: Target is a Chrome memory information-leak via a crafted HTML page. All five neighbours are Chrome memory / V8 out-of-bounds issues via crafted HTML. Clean same-class match. Directly parallel to the Chrome few-shot example.
...
NOTES: Target is authenticated OS command execution in Zoho ManageEngine ADSelfService Plus. All five neighbours are Zoho ManageEngine products spanning command execution, file-upload RCE and auth bypass. Strong product grounding with an execution core.
...
NOTES: Target is code execution via malicious image parsing in ExifTool. Neighbours are image / file-parsing memory and code-execution bugs. Same file-parsing-to-execution family, grounded.
...
NOTES: Target is command injection in Mitel MiVoice Edge Gateway. Neighbours span prototype pollution, XSS, SQL injection, buffer overflow and command injection. Several are injection-family but the spread is wide. High distance (0.4328) reflects genuine class mixing.
